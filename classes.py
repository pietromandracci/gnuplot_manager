...
class _PlotWindow:
    """ This class defines a single plot window, which can contain several plotted curves. """
    ...
    def __init__(self,
        ...
        """ An instance of this class contains a plot window and the gnuplot process used to plot on it.
            ...
            self.window_number:   an integer number that identifies the plot window
            self.gnuplot_process: gnuplot process (instance of subprocess.Popen)
            self.term_type:       the type of gnuplot terminal started (e.g. 'x11' or 'wxt')
            self.persistence:     True if the window is persistent
            self.filename_out:    name of the file to which gnuplot output is redirected 
            self.filename_err:    name of the file to which gnuplot error messages are redirected
            ...
            self.data_filenames:  list containing the names of the data files (strings)
            self.functions:       list containing the functions plotted (strings)
        ...
        if redirect_output is True:
            ...
            self.filename_out = path.join( DIRNAME_OUT, FILENAME_OUT + '_w_' + str(self.window_number) )
            self.filename_err = path.join( DIRNAME_ERR, FILENAME_ERR + '_w_' + str(self.window_number) )
            if self.title is not None:
                self.filename_out += '(' + self._correct_filename(self.title) + ')'
                self.filename_err += '(' + self._correct_filename(self.title) + ')'
    ...
    def _quit_gnuplot(self):
        ...
        try:
            gnuplot_last_output = self.gnuplot_process.communicate(input='quit', timeout=TIMEOUT_QUIT)
    ...
    def _add_curves(self, data_list, replot=False):
        """ Add one or more curves from data to the plot window. 
            ...
            data_list:    a list in one of the following forms:
                          ...
                          The form must be consisted with the type of plot that was defined
                          at the plot window creation, otherwise an error message is returned
            replot:       if True, the curves are plotted without erasing previously plotted ones
